...
export async function POST(req: NextRequest) {
    ...
    if (!email || !password || !alphaKey) {
        return NextResponse.json({ message: "Email or password not provided" });
    }
    if (alphaKey !== process.env.ALPHA_KEY) {
        return NextResponse.json({ message: "Not Authorized. Please Request Alpha Key" });
    }
    ...
    if (existingUser) {
        return NextResponse.json({ message: "Try a different email" });
    }
